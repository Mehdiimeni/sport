<?php
///controller/user/user_position_details.php
...
$comment->part_id = $position_id;

$positionDetail = $user->getPositionById($position_id)->fetch_assoc();
$userProfile = $user->getUserImageById($positionDetail['user_id']);

function getValue($fieldName, $positionDetail)
{
    return isset($positionDetail[$fieldName]) ? $positionDetail[$fieldName] : '';
}
...
$allFileData = $fileManager->getFileManageByPart($position_id, $part_name);
...
window.location.replace('position?position_id='+$position_id); 
    </script>
HTML;
...
HTML;
    ...
    <script>
        ...
        if ('$message' === 'Data inserted successfully.') {
            window.location.replace ('user_position?id='+$position_id); 
        }
    </script>
HTML;
    }
